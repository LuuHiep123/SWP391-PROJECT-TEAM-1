...
public class StudentExamDAO {
    ...
    private static final String SORT_BY_DATE = "SELECT exam.Subject_ID, sl.Start_Time, sl.End_Time,ro.[Date],ro.NumRoom,ro.Campus,su.Form"
            + " " + "FROM Student_Exam_Schedule exam"
            + " " + "JOIN Semester se ON exam.SE_ID = se.SE_ID"
            + " " + "JOIN Room ro ON exam.Room_ID = ro.Room_ID"
            + " " + "JOIN Slot sl ON exam.Slot_ID = sl.Slot_ID"
    ...
    public List<StudentExamDTO> SortByDate(String email) throws ClassNotFoundException, SQLException {
        Connection conn = DBUtils.getConnection();
        PreparedStatement ptm = null;
        ResultSet rs = null;
        List<StudentExamDTO> ListExam = new ArrayList<>();
        ...
        try {
            if (conn != null) {
                ptm = conn.prepareStatement(SORT_BY_DATE);
                ptm.setString(1, email);
                rs = ptm.executeQuery();
                while (rs.next()) {
                    String SubjectID = rs.getString("Subject_ID");
